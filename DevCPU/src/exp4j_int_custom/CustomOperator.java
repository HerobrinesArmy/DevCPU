 ...
 * The applyOperation(int[] values) will have to be implemented by users of this class. <br/>
 * <b>Example</b><br/>
 * <code><pre>{@code} 
 *      CustomOperator greaterEq = new CustomOperator(">=", true, 4, 2) {
 *            int applyOperation(int[] values) {
...
public abstract class CustomOperator {
	...
	/**
	 * Apply the custom operation on the two operands and return the result as an int An example implementation for a
	 ...
	 */
	protected abstract int applyOperation(int[] values);
}
