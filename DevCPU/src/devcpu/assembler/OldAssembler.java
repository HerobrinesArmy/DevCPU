...
//        }
  ...
  public void include(String resourceFile) throws Exception {
    ...
    BufferedReader br = new BufferedReader(new InputStreamReader(OldAssembler.class.getResourceAsStream("/" + resourceFile)));
    String line = "";
    int lines = 0;
  ...
  private class Macro
  {
    ...
    public void insert(String[] values) { String[] ts = new String[this.tokens.size()];
      ...
      OldAssembler.this.handleTokens(ts);
    }
  }

  private class Position
  {
    public final OldAssembler.Scope scope;
    public final int pos;

    public Position(OldAssembler.Scope scope, int pos)
    {
      this.scope = scope;
      this.pos = pos;
    }
  ...
  private class Scope
  {
    public Map<String, OldAssembler.Position> labelPositions = new HashMap<String, OldAssembler.Position>();
    public Scope parent;
    ...
    public OldAssembler.Position findLabel(String label) {
      return findLabel(label, new ArrayList<Scope>());
    }

    public OldAssembler.Position findLabel(String label, List<Scope> testedScopes) {
      if (testedScopes.contains(this)) return null;
      testedScopes.add(this);
      if (this.labelPositions.containsKey(label)) return (OldAssembler.Position)this.labelPositions.get(label);
      for (int i = 0; i < this.inheritedScopes.size(); i++) {
        OldAssembler.Position p = ((Scope)this.inheritedScopes.get(i)).findLabel(label, testedScopes);
        if (p != null) return p;
      }
  ...
  public static void main(String[] args) {
  	OldAssembler assembler = new OldAssembler(new char[65536]);
  	try {
